...
int main() {
    ...
    const char* sendBuffer = "Добро пожаловать на сервер"; // буфер, в котором хранится сообщение сервера

    int result = WSAStartup(MAKEWORD(2, 2), &wsaData); // инициализация библиотек
    if (result != 0) {
        cout << "Ошибка WSAStartup с результатом: " << result << endl;
        return 1;
    }
    ...
    if (ConnectSocket == INVALID_SOCKET) {
        cout << "Ошибка принятия входящего соединения : " << WSAGetLastError() << endl;
        closesocket(ListenSocket);
        freeaddrinfo(addrResult);
        WSACleanup();
    ...
    do {//Цикл для приема данных от клиента.
        ...
        if (result > 0) {
            ...
            if (result == SOCKET_ERROR) {
                cout << "Ошибка в отправки сообщения: " << result << endl;
...
//how: Это параметр, определяющий, какое действие должно быть выполнено над сокетом.Может принимать одно из следующих значений :
    ...
    if (result == SOCKET_ERROR) {
        cout << "Сбой завершения работы " << result << endl;
        closesocket(ConnectSocket);
        freeaddrinfo(addrResult);
        WSACleanup();
        return 1;
